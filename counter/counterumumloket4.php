...
<div class="sidebar glass">
  ...
  while($r=mysqli_fetch_array($hld)){
    echo "<tr><td>{$r[0]}</td>
      <td><form method=POST action='aksi_panggil.php?act=clearer&id={$r[0]}&loket=4'><button>Clear</button></form></td>";
    if($act!="call") echo "<td><a href='counterumumloket4.php?act=callback&no={$r[0]}'><button>Callback</button></a></td>";
    echo "</tr>";
  } ?>
  </table>
</div>
<div class="main">
  ...
  switch($act){
    case "call":
      ...
      if($p){ $id=$p['id']; mysqli_query($conn,"UPDATE tbl_bpjs SET panggil=1,loket=4 WHERE id=$id");
        echo "<h1>B$id</h1><div class='actions'>
          <form method=POST action='aksi_panggil.php?act=clear&id=$id&loket=4'><button>Clear</button></form>
          <form method=POST action='aksi_panggil.php?act=hold&id=$id&loket=4'><button>Hold</button></form>
          <button onclick='playAudio($id,4)'>Play Audio</button></div>";
    ...
    case "callback":
      ...
      if($no>0){ $p=mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM tbl_bpjs WHERE status=2 AND id=$no AND panggil=0"));
        if($p){ $id=$p['id']; mysqli_query($conn,"UPDATE tbl_bpjs SET panggil=2,loket=4 WHERE id=$id");
          echo "<h1>B$id</h1><div class='actions'>
            <form method=POST action='aksi_panggil.php?act=clear&id=$id&loket=4'><button>Clear</button></form>
            <form method=POST action='aksi_panggil.php?act=hold&id=$id&loket=4'><button>Hold</button></form>
            <button onclick='playAudio($id,4)'>Play Audio</button></div>";
  ...
  <script src="audioloket4.js"></script>
</div>
</body>
</html>
