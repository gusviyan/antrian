...
<div class="w3-main" style="margin-left:300px">
  ...
  <!-- Photo grid -->
    ...
    <div class="w3-half" style="border: 5px solid black; width: 50%; height:750px">
		...
		if ( $act == "call") {
			$cek = mysql_query ("SELECT * FROM tbl_bpjs_rajal WHERE status = 2 AND panggil = 2 AND loket = 3 LIMIT 1");
			...
			if ($pgl == 0){
				...
				mysql_query("UPDATE tbl_bpjs_rajal SET  panggil = 1, loket = 3 WHERE id = $pgl[0]");
				mysql_query("UPDATE tbl_bpjs_rajal SET  panggil = 0, loket = 0 WHERE status = 2 AND loket = 3");
				if ($pgl != ''){
					echo"
					...
					<form method=POST action='aksi_panggil.php?act=clear&id=$pgl[0]&loket=3'>
					...
					<form method=POST action='aksi_panggil.php?act=hold&id=$pgl[0]&loket=3'>
					...
					<div class='w3-display-bottomleft w3-large' style='padding-bottom: 200px;'>
					<p style='font-size:200%;'><a href='counterumumloket3.php?act=default'><button onclick='document.getElementById('rajal').style.display=block' class='w3-button w3-black'>Back</button></a></p>
					</div>
					";
				}
			} else {
			...
			mysql_query("UPDATE tbl_bpjs_rajal SET  panggil = 2, loket = 3 WHERE id = $pgl[0]");
