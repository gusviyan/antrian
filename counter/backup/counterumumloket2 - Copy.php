	...
	<?php
		...
		while ($hldlist = mysql_fetch_array($hld)) {
			...
			if ( $act != "call"){
				echo"
			<td >
			<a href='counterumumloket2.php?act=callback&no=$hldlist[0]'><button class='w3-button w3-black' onclick='playAudio($hldlist[0],2)'>Callback</button> </a>
			</td>
			</tr>
			";
			}
  ...
  <div class="w3-row" style="margin-top:0">
    <div class="w3-half" style="border: 5px solid black; width: 50%; height:750px">
		<div class="w3-top w3-large" style="padding-left: 5px;">
			<p style="font-size:200%"><a href='counterumumloket2.php?act=call'><button onclick="document.getElementById('rajal').style.display='block'" class="w3-button w3-black">Call</button></a>
			</p>
		</div>
		...
		if ( $act == "call") {
			$cek = mysql_query ("SELECT * FROM tbl_umum WHERE status = 2 AND panggil = 2 AND loket = 2 LIMIT 1");
			$pgl = mysql_num_rows ($cek);
			if ($pgl == 0){
				$cek = mysql_query ("SELECT * FROM tbl_umum WHERE status = 0 AND panggil = 0 LIMIT 1");
				$pgl = mysql_fetch_array ($cek);
				mysql_query("UPDATE tbl_umum SET  panggil = 1, loket = 2 WHERE id = $pgl[0]");
				mysql_query("UPDATE tbl_umum SET  panggil = 0, loket = 0 WHERE status = 2 AND loket = 2");
				if ($pgl != ''){
					echo"
					...
					<form method=POST action='aksi_panggil.php?act=clear&id=$pgl[0]&loket=2'>
					...
					<form method=POST action='aksi_panggil.php?act=hold&id=$pgl[0]&loket=2'>
					...
					<div class='w3-display-bottomleft w3-large' style='padding-bottom: 200px;'>
					<p style='font-size:200%;'><a href='counterumumloket2.php?act=default'><button onclick='document.getElementById('rajal').style.display=block' class='w3-button w3-black'>Back</button></a></p>
					</div>
					";
				}
			} else {
		...
		$pgl = mysql_fetch_array ($cek);
			mysql_query("UPDATE tbl_umum SET  panggil = 0, loket = 0 WHERE status = 2 AND loket = 2");
			mysql_query("UPDATE tbl_umum SET  panggil = 2, loket = 2 WHERE id = $pgl[0]");
