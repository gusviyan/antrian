...
<div class="sidebar glass">
  ...
  while($r=mysqli_fetch_array($hld)){
    echo "<tr><td>{$r[0]}</td>
      <td><form method=POST action='aksi_panggil.php?act=clearer&id={$r[0]}&loket=1'><button>Clear</button></form></td>";
    if($act!="call") echo "<td><a href='counterumumloket1.php?act=callback&no={$r[0]}'><button>Callback</button></a></td>";
    echo "</tr>";
  } ?>
  </table>
</div>
<div class="main">
  <?php
  $cek=mysqli_query($conn,"SELECT id FROM tbl_umum WHERE status=0 AND panggil=0 LIMIT 1");
  ...
  switch($act){
    case "call":
      $p=mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM tbl_umum WHERE status=0 AND panggil=0 ORDER BY id ASC LIMIT 1"));
      if($p){ $id=$p['id']; mysqli_query($conn,"UPDATE tbl_umum SET panggil=1,loket=1 WHERE id=$id");
        echo "<h1>A$id</h1><div class='actions'>
          <form method=POST action='aksi_panggil.php?act=clear&id=$id&loket=1'><button>Clear</button></form>
          <form method=POST action='aksi_panggil.php?act=hold&id=$id&loket=1'><button>Hold</button></form>
          <button onclick='playAudio($id,1)'>Play Audio</button></div>";
      } else echo "<h1>Belum ada antrian</h1>"; break;
    case "callback":
      $no=intval($_GET['no']??0);
      if($no>0){ $p=mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM tbl_umum WHERE status=2 AND id=$no AND panggil=0"));
        if($p){ $id=$p['id']; mysqli_query($conn,"UPDATE tbl_umum SET panggil=2,loket=1 WHERE id=$id");
          echo "<h1>A$id</h1><div class='actions'>
            <form method=POST action='aksi_panggil.php?act=clear&id=$id&loket=1'><button>Clear</button></form>
            <form method=POST action='aksi_panggil.php?act=hold&id=$id&loket=1'><button>Hold</button></form>
            <button onclick='playAudio($id,1)'>Play Audio</button></div>";
