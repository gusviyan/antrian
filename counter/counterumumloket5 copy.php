...
{
	setTimeout( function() {
	  $('#auto').fadeIn("slow").load('load5.php').fadeOut("slow");
	  refresh();
	}, 2000);
}
  ...
  <h3 class="w3-padding-10 w3-center"><b>ANTRIAN HOLD</b></h3>
	...
	<?php
		...
		while ($hldlist = mysqli_fetch_array($hld)) {
			...
			<td > 
			<form method=POST action='aksi_panggil.php?act=clearer&id={$hldlist[0]}&loket=5'>
			...
			if ( $act != "call"){
				echo"
			<td >
			<a href='counterumumloket5.php?act=callback&no={$hldlist[0]}'><button class='w3-button w3-black' onclick='playAudio({$hldlist[0]},2)'>Callback</button> </a>
			</td>
			</tr>
			";
			}
  ...
  <div class="w3-row" style="margin-top:0">
	<div class="w3-half" style="border: 5px solid black; width: 50%; height:750px">
		<div class="w3-top w3-large" style="padding-left: 5px;">
			<p style="font-size:200%"><a href='counterumumloket5.php?act=call'><button onclick="document.getElementById('rajal').style.display='block'" class="w3-button w3-black">Call</button></a>
			</p>
		</div>
...
$cekQueue = mysqli_query($conn, "SELECT id FROM tbl_cs WHERE status = 0 AND panggil = 0 LIMIT 1");
$hasQueue = mysqli_num_rows($cekQueue) > 0;

$callLink = $hasQueue
    ? "counterumumloket5.php?act=call"
    : "counterumumloket5.php?act=default";
?>
...
switch ($act) {
    case "call":
        ...
        $cek = mysqli_query($conn, "SELECT id FROM tbl_cs 
                                    WHERE status = 2 AND panggil = 2 AND loket = 5
                                    LIMIT 1");
        ...
        if ($jml === 0) {
            ...
            if ($pgl) {
                ...
                mysqli_query($conn, "UPDATE tbl_cs SET panggil = 0, loket = 0 
                                     WHERE status = 2 AND loket = 5");

                echo "
                <div id='rajal' style='display: block;'>    
                    <center>
                        <h1 style='font-size:1200%; padding-top: 100px'>CS{$id}</h1>
                    </center>

                    <div class='w3-display-bottomleft w3-large' style='padding-left:100px; padding-bottom:200px;'>
                        <form method='POST' action='aksi_panggil.php?act=clear&id={$id}&loket=5'>
                    ...
                    <div class='w3-display-bottomleft w3-large' style='padding-bottom:200px;'>
                        <form method='POST' action='aksi_panggil.php?act=hold&id={$id}&loket=5'>
                            <p style='font-size:200%;'>
...
?>
		...
		<center>
		<h1 class='w3-display-bottomleft' style='font-size:300%;padding-bottom: 7%;'> CS
