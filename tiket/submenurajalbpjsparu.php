...
function openWin(n,x) {
	...
	if (x == 1){
		dokter = "dr. Yenni Sari Siregar, Sp.P"
	} else if (x == 2) {
		dokter = "Dr. dr. Mukhtar Ikhsan, Sp.P(K), MARS"
	} 
   myWindow = window.open("", "myWindow", "width=2,height=1");
   myWindow.document.write("<center><h3>Rawat Jalan BPJS</h3> <h3>"+ "<h3> Poli Paru / " + dokter +"</h3>"+"</h3> <h1 style='font-size:400%;'>B" + n +"</h1>" + "<p> Jika nomor terlewat akan dipanggil setelah 3 nomor</p>" + result + "</center>");
   myWindow.print();
   myWindow.close();
}
</script>
  ...
  if ($date == "Monday" || $date == "Friday"){
	  echo"
	   	<div class='w3-display-middle1' id='feature3' >
				<a href='submenurajalbpjsparu.php?act=dryenni' class='btn btn-lg btn-default wow fadeInUp'>
					<p>dr. Yenni Sari Siregar, Sp.P </p></a>
			</div>
  ";
  } else{ 
	  echo"
		<div class='w3-display-middle1' id='feature3' >
			<a href='submenurajalbpjsparu.php?act=tdkpraktek' class='w3-black btn btn-lg btn-default wow fadeInUp'>
			<p>dr. Yenni Sari Siregar, Sp.P</p></a>
	  </div>
  ";
  }
  
  /*$sqlparu = mysql_query("SELECT * FROM tbl_bpjs_rajal WHERE poli = 'Poli Paru' AND dokter = '2'");
  $rowparu = mysql_num_rows($sqlparu); */
  if ($date == "Wednesday" || $date == "Saturday" || $date == "Tuesday"){
	 //if ($rowparu < 10 ) {
		 echo"	 
		 <div class='w3-display-middle2' id='feature3' >
			<a href='submenurajalbpjsparu.php?act=drmukthar' class='btn btn-lg btn-default wow fadeInUp'>
				<p>Dr. dr. Mukhtar Ikhsan, Sp.P(K), MARS</p>
			</a>
		  </div>";
	/* } else {
		 echo"	 
...
if ($_GET['act'] == "dryenni") {
	...
	$cekpoli = mysql_query("SELECT * FROM tbl_bpjs_rajal WHERE poli = 'Poli Paru' AND dokter = '1' ");
	$numrow = mysql_num_rows($cek);
	$numrowpoli = mysql_num_rows($cekpoli);
	$tambah = $numrow + 1;
	$tambahpoli = $numrowpoli + 1;
	mysql_query("INSERT INTO tbl_bpjs_rajal (id,poli,id_poli,dokter) VALUES ($tambah,'Poli Paru',$tambahpoli,'1')");
	echo "
	...
	<center>
	  <div class='w3-modal-content w3-animate-zoom'>
		...
		<h1 style='color:black;'>Poli Paru</h1>
	  ...
	  ";
} else if ($_GET['act'] == "drmukthar") {
	$cek = mysql_query("SELECT * FROM tbl_bpjs_rajal");
	$cekpoli = mysql_query("SELECT * FROM tbl_bpjs_rajal WHERE poli = 'Poli Paru' AND dokter = '2' ");
	$numrow = mysql_num_rows($cek);
	$numrowpoli = mysql_num_rows($cekpoli);
	$tambah = $numrow + 1;
	$tambahpoli = $numrowpoli + 1;
	mysql_query("INSERT INTO tbl_bpjs_rajal (id,poli,id_poli,dokter) VALUES ($tambah,'Poli Paru',$tambahpoli,'2')");
	echo "
	<div id='rajalpd' class='w3-modal' style='display: block'>
	<center>
	  <div class='w3-modal-content w3-animate-zoom'>
