...
function openWin(n,x) {
	...
	if (x == 1){
		dokter = "dr. Agus Prasetyo, Sp.KFR"
	} else if (x == 2) {
		dokter = "Therapy"
	} 
   myWindow = window.open("", "myWindow", "width=2,height=1");
   myWindow.document.write("<center><h3>Rawat Jalan BPJS</h3> <h3>"+ "<h3> Poli Rehab Medis / " + dokter +"</h3>"+"</h3> <h1 style='font-size:400%;'>B" + n +"</h1>" + "<p> Jika nomor terlewat akan dipanggil setelah 3 nomor</p>" + result + "</center>");
   myWindow.print();
   myWindow.close();
}
</script>
...
<svg preserveAspectRatio="none" viewBox="0 0 100 100" height="150" width="100%" version="1.1" xmlns="http://www.w3.org/2000/svg" class="svgcolor-light">
  ...
  <?php
	...
	$date = date("l");
	$sqlfisio = mysql_query("SELECT * FROM tbl_bpjs_rajal WHERE poli = 'Poli Rehab Medis' AND dokter = '1'");
	$rowfisio = mysql_num_rows($sqlfisio);
	if ($date == "Monday" || $date == "Tuesday" || $date == "Wednesday" || $date == "Thursday"){
		if ($rowfisio < 17 ) {
			echo"
		   <div class='w3-display-middleleft' id='feature3'>
				<a href='submenurajalbpjsfisio.php?act=dragus' class='btn btn-lg btn-default wow fadeInUp'>
				<p style='font-size:50px;'>dr. Agus Prasetyo, Sp.KFR</p></a>
		 	 </div>
		  ";
		}else{
			echo"
		   <div class='w3-display-middleleft' id='feature3'>
				<a href='submenurajalbpjsfisio.php?act=tutup' class='w3-black btn btn-lg btn-default wow fadeInUp'>
					<p style='color:white;font-size:50px;'>dr. Agus Prasetyo, Sp.KFR</p></a>
		 	 </div>
		  ";
		}
	} else {
		echo"
		   <div class='w3-display-middleleft' id='feature3'>
				<a href='submenurajalbpjsfisio.php?act=tdkpraktek' class='w3-black btn btn-lg btn-default wow fadeInUp'>
					<p style='color:white;font-size:50px;'>dr. Agus Prasetyo, Sp.KFR</p></a>
		 	 </div>
		  ";
	}
  ...
  <a href='submenurajalbpjsfisio.php?act=petugas' class='btn btn-lg btn-default wow fadeInUp'>
		<p>Therapy</p>
  </a>
  </div>
  ";
...
if ($_GET['act'] == "dragus") {
	...
	$cekpoli = mysql_query("SELECT * FROM tbl_bpjs_rajal WHERE poli = 'Poli Rehab Medis' AND dokter = '1' ");
	$numrow = mysql_num_rows($cek);
	$numrowpoli = mysql_num_rows($cekpoli);
	$tambah = $numrow + 1;
	$tambahpoli = $numrowpoli + 1;
	mysql_query("INSERT INTO tbl_bpjs_rajal (id,poli,id_poli,dokter) VALUES ($tambah,'Poli Rehab Medis',$tambahpoli,'1')");
	echo "
	...
	<center>
	  <div class='w3-modal-content w3-animate-zoom'>
		...
		<h1 style='color:black;'>Poli Rehab Medis</h1>
	...
	$cek = mysql_query("SELECT * FROM tbl_bpjs_rajal");
	$cekpoli = mysql_query("SELECT * FROM tbl_bpjs_rajal WHERE poli = 'Poli Rehab Medis' AND dokter = '2' ");
	$numrow = mysql_num_rows($cek);
	$numrowpoli = mysql_num_rows($cekpoli);
	$tambah = $numrow + 1;
	$tambahpoli = $numrowpoli + 1;
	mysql_query("INSERT INTO tbl_bpjs_rajal (id,poli,id_poli,dokter) VALUES ($tambah,'Poli Rehab Medis',$tambahpoli,'2')");
	echo "
	<div id='rajalpd' class='w3-modal' style='display: block'>
	<center>
	  <div class='w3-modal-content w3-animate-zoom'>
